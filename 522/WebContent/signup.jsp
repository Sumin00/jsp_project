...
<script type="text/javascript">
	...
	function checkSignup(){
		...
		if (!regx.test(signup.userID.value)){
	...
	function idcheck(userID){
		...
		function checkEmail(){
			...
			if (signup.selectEmail.value == "1"){
				signup.email2.focus();
			}
			else {
				signup.email2.readonly = true;
				signup.email2.value = signup.selectEmail.value;
			}
	...
	<div id="background">
		<div>
			<span id="title">회원가입</span> 		<br> <br>
			Join Us, Ediya Coffee <br>
			이디야 멤버스 가입을 위한 약관 동의 및 본인 인증단계입니다.
		</div>
	</div>
	
	<br><br><br>
	
<form method="post" action="signupAction.jsp" name="signup" onsubmit="return checkSignup()">
	<div>
		
		<table id="t1">
			<tr>
				<td><strong>아이디</strong></td>
				<td> <input type="text" name="userID" size="20" placeholder="영문/숫자 조합"></td>
				<td> <input type="button" value="아이디 중복확인" onclick="idcheck(this.form.userID.value)"></td>
			</tr>
			<tr>
		...
		<strong style="font-size:25px;">서비스 이용약관</strong>	<br><br>
		
		<button id="button1" onclick="agreeAll();" type="button"><img alt="" src="image/check.png"></button><strong>전체동의</strong> ( <span id="strong">&#8251; 선택동의 사항이 포합되어 있습니다.</span> )
		<br><br>
		
		<table>
			<tr>
				<td>이디야 멤버스 서비스 이용약관 동의 (필수)
					<input type="checkbox" name="agree" value="a1"> </td>
			</tr>
			<tr>
				<td>
					<textarea style="resize: none; width: 500px; height: 200px" readonly>
제 1장 총칙
제 1 조 (목적)
...
2. “e나라 표준인증”에서 제공하는 국가표준, 인증제도, 기술기준, 인증지원 정보를 말합니다.
3. "회원"이라 함은 서비스를 이용하기 위하여 당 사이트에 개인정보를 제공하여 아이디(ID)와 비밀번호를 부여 받은 자를 말합니다.
4. “비회원”이하 함은 회원으로 가입하지 않고 " e나라 표준인증"에서 제공하는 서비스를 이용하는 자를 말합니다.
...
제 7 장 " e나라 표준인증" 게시물 운영정책
제 20 조 (운영정책)
"e나라 표준인증" 각종 게시물에 대한 운영정책은 방송통신심의위원회의 정보통신에 관한 심의규정에 기반하며 이를 위반할 경우, "e나라 표준인증" 운영정책에 의해 관련 게시물은 예고 없이 삭제, 이동될 수 있으며, 게시자(회원)는 아이디 이용제한 등 "e나라 표준인증" 이용에 제한을 받을 수 있습니다.
...
1. 욕설/비속어 및 분란을 조장하는 게시물
- 욕설 및 비속어가 담겨있거나, 연상시키는 내용
- 이유 없이 회원 간의 분란을 발생시킬 여지가 있는 내용의 게시물 또는 댓글
2. 게시판 및 자료실과 관련 없는 게시물
...
- 공인이나 특정이슈와 관련한 당사자 또는 지인, 주변인 등을 사칭하여 게시물을 작성하거나, "e나라 표준인증" 운영자를 사칭하여 작성된 내용
...
(시행일) 이 약관은 2016년 12월 30일부터 적용되며, 종전 약관은 본 약관으로 대체되며, 개정된 약관의 적용일 이전 가입자도 개정된 약관의 적용을 받습니다.
				...
				<tr>
				<td>개인정보 수집 및 이용 동의 (필수)
					<input type="checkbox" name="agree" value="a2"> </td>
			</tr>
			<tr>
				<td>
					<textarea style="resize: none; width: 500px; height: 200px" readonly>
...
(시행일) 이 약관은 2016년 12월 30일부터 적용되며, 종전 약관은 본 약관으로 대체되며, 개정된 약관의 적용일 이전 가입자도 개정된 약관의 적용을 받습니다.
				...
				<tr>
				<td>마케팅 활용 수신 동의 (선택)
					<input type="checkbox" name="agree" value="a3"> </td>
			</tr>
			<tr>
				<td>
					<textarea style="resize: none; width: 500px; height: 200px" readonly>
...
2. 마케팅 정보 제공
주식회사 올룰로는 KICKGOING(킥고잉) 서비스를 운용하는 과정에서 각종 정보를 서비스 화면, SMS, 푸시 알림, 이메일 등의 방법으로 회원에게 제공할 수 있으며, 결제 안내 등 의무적으로 안내해야 하는 정보성 내용 및 일부 혜택성 정보는 수신동의 여부와 무관하게 제공합니다.

3. 수신 동의 및 철회
주식회사 올룰로에서 제공하는 마케팅 정보를 원하지 않을 경우 ‘설정 > 마케팅 수신 동의 설정’에서 철회를 요청할 수 있습니다. 또한 향후 마케팅 활용에 새롭게 동의하고자 하는 경우 ‘설정 > 마케팅 수신 동의 설정’에서 동의하실 수 있습니다.
						</textarea>	
					</td>
				</tr>	
				<tr>
					<td><span id="strong">&#8251; 선택사항에 동의하지 않으셔도 서비스 가입 및 이용이 가능하나, 동의하지 않을 경우 제공 가능한 관련 편의사항이 제한될 수 있습니다.</span></td>
				</tr>	
		</table>
		
		<hr>
		
		<input id="button2" type="reset" value="다시작성">
		<input id="button2" type="submit" value="가입하기">
